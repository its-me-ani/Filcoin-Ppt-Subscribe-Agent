...
import { saveFileOnChain, getExplorerUrl } from "../../../services/blockchain";
...
const Menu: React.FC<{
  ...
  const sendEmail = async () => {
    if (isPlatform("hybrid")) {
      try {
        ...
        // Use invoiceId or selectedFile as the invoice identifier
        const invoiceIdentifier = invoiceId || selectedFile || "Invoice";
        const filename = `${invoiceIdentifier}.html`;

        // Write HTML to a temporary file to avoid TransactionTooLargeException
        // Android has a ~1MB limit for IPC transactions, and large base64 data
        // in plugin calls can exceed this when the app pauses
        // Use External directory for better FileProvider compatibility with email clients
        const tempFile = await Filesystem.writeFile({
          path: filename,
          data: content,
        ...
        // Convert file URI to absolute path (remove file:// prefix)
        // EmailComposer expects a plain path for "absolute" type
        const absolutePath = tempFile.uri.replace('file://', '');
        ...
        await EmailComposer.open({
          ...
          attachments: [{ type: "absolute", path: absolutePath, name: filename }],
          subject: "Here is your Invoice",
          isHtml: true,
        });

        // Clean up the temp file after a delay (email client should have copied it)
        setTimeout(async () => {
          try {
  ...
  // Save to Blockchain handler (content → Pinata IPFS → CID → on-chain)
  const doSaveToBlockchain = async () => {
    try {
      // Check wallet connection
      if (!isConnected) {
        setToastMessage("Connecting wallet...");
        setShowToast1(true);
        await connectWallet();
        // After connect, user should try again
        setToastMessage("Wallet connected! Please try 'Save to Blockchain' again.");
        setShowToast1(true);
        return;
      }
      ...
      if (balance < 1) {
        setToastMessage("You need at least 1 PPT token to save files on-chain.");
        setShowToast1(true);
        return;
      }

      setIsSavingToChain(true);
      setToastMessage("Uploading to IPFS & saving on-chain...");
      setShowToast1(true);

      // Get current spreadsheet content
      const content = AppGeneral.getSpreadsheetContent();
      if (!content || content.trim() === "") {
        setToastMessage("No content available to save.");
        setShowToast1(true);
      ...
      // Save to IPFS + blockchain (new flow)
      const invoiceIdentifier = invoiceId || selectedFile || "Invoice";
      const { txHash, cid } = await saveFileOnChain(encodedContent, {
        name: invoiceIdentifier,
        invoiceId: invoiceIdentifier,
      });

      setToastMessage(`Saved! IPFS: ${cid.slice(0, 12)}... | Tx: ${txHash.slice(0, 10)}...`);
      setShowToast1(true);
    } catch (error: any) {
      const message = error?.reason || error?.message || "Failed to save to blockchain.";
  ...
  const getMenuButtons = () => {
    ...
    baseButtons.push(
      ...
      {
        text: isSavingToChain ? "Saving to Blockchain..." : "Save to Blockchain",
        icon: cloudUpload,
        handler: () => {
          doSaveToBlockchain();
        },
